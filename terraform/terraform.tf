...
locals {
  common_vars = {
    ...
    PKR_VAR_linux_distro  = var.linux_distro
    PKR_VAR_image_name    = "caravan_${var.linux_distro}"
  }
  build_image_name = "caravan_${var.linux_distro}"
}
...
resource "null_resource" "run_packer_google" {
  ...
  triggers = {
    ...
    "changes-in-packer" : filemd5("${path.module}/../packer/${var.linux_distro}-gcp.pkr.hcl")
  }
  provisioner "local-exec" {
    working_dir = "${path.module}/../packer"
    command     = "packer build -force -only=${join(",", var.builds)} ${var.linux_distro}-gcp.pkr.hcl"
    environment = merge({
...
resource "null_resource" "run_packer_aws" {
  ...
  triggers = {
    ...
    "changes-in-packer" : filemd5("${path.module}/../packer/${var.linux_distro}-aws.pkr.hcl")
  }
  provisioner "local-exec" {
    working_dir = "${path.module}/../packer"
    command     = "env && packer build -force -only=${join(",", var.builds)} ${var.linux_distro}-aws.pkr.hcl"
    environment = merge({
...
resource "null_resource" "run_packer_azure" {
  ...
  triggers = {
    ...
    "changes-in-packer" : filemd5("${path.module}/../packer/${var.linux_distro}-azure.pkr.hcl")
  }
  provisioner "local-exec" {
    working_dir = "${path.module}/../packer"
    command     = "packer build -force -only=${join(",", var.builds)} ${var.linux_distro}-azure.pkr.hcl"
    environment = merge({
